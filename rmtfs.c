...
static char rmtfs_read_singlesim_flag(void)
{
    char c = 0;
    FILE *f = fopen(SINGLE_SIM_FILE, "r");
...
static void rmtfs_odm_get_single_sim_tag(int sock, struct qrtr_packet *pkt)
{
	struct rmtfs_single_sim_tag_resp resp = {};
	struct rmtfs_single_sim_tag_req req = {};
	DEFINE_QRTR_PACKET(resp_buf, 256);
	struct rmtfd *rmtfd;
	unsigned txn;
	ssize_t len;
	...
	ret = qmi_decode_message(&req, &txn, pkt, QMI_REQUEST,
				QMI_RMTFS_ODM_GET_SINGLE_SIM_TAG,
				rmtfs_single_sim_tag_req_ei);
	if (ret < 0) {
		qmi_result_error(&resp.result, QMI_RMTFS_ERR_MALFORMED_MSG);
		goto respond;
	}
	...
	resp.single_sim_tag = (c == '1') ? 1 : 2;
	resp.single_sim_tag_valid = true;

respond:
	dbgprintf("[RMTFS] single_sim_tag %d => %d (%d:%d)\n", req.caller_id,
		  resp.single_sim_tag, resp.result.result, resp.result.error);

	len = qmi_encode_message(&resp_buf,
				QMI_RESPONSE, QMI_RMTFS_ODM_GET_SINGLE_SIM_TAG, txn,
				&resp, rmtfs_single_sim_tag_resp_ei);
	if (len < 0) {
		fprintf(stderr, "[RMTFS] failed to encode single-sim-tag-response: %s\n",
			strerror(-len));
		return;
	}
	...
	ret = qrtr_sendto(sock, pkt->node, pkt->port,
			 resp_buf.data, resp_buf.data_len);
	if (ret < 0)
		fprintf(stderr, "[RMTFS] failed to send single-sim-tag-response: %s\n",
			strerror(-ret));
}
...
static int handle_rmtfs(int sock)
{
	...
	case QRTR_TYPE_DATA:
		...
		case QMI_RMTFS_GET_DEV_ERROR:
			...
			break;
		case QMI_RMTFS_ODM_GET_SINGLE_SIM_TAG:
			rmtfs_odm_get_single_sim_tag(sock,&pkt);
			break;
		default:
